...
def validate_input(ds : xr.Dataset)->None:
   """
    Validates an xarray Dataset against expected coordinates, 
    variables, and value constraints.

    This function checks whether the dataset:
    - Contains all required coordinates and data variables.
    - Has latitude, longitude, and level values within expected ranges.
    - Does not contain NaN values in the expected data variables.
    - Ensures data values fall within their defined valid ranges.
    ...
    ------
    ValueError
        If any validation check fails, detailing the issues found.
    """
   ...
   if coords != EXPECTED_COORDS:
      ...
      checks.append((f'Unexpected coordinates.' 
   ...
   if data_vars != EXPECTED_VARS:
      ...
      checks.append((f'Unexpected data variables.'
